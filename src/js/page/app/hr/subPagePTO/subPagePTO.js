...
const PageContent = (props) => {
    ...
    switch (_pstate.childPage) {
        case _childPages.all:
            ...
            // last_update_by VARCHAR NOT NULL default 'admin',
            // status integer NOT NULL default 1,
            // comment text,
            // application_id VARCHAR NOT NULL Unique,
            // user_name VARCHAR NOT NULL,
            // type VARCHAR NOT NULL, --leave,office_supplies,resignation
            // description text not null,
            //     approve_by VARCHAR NOT NULL,
            //     approve_status VARCHAR NOT NULL default 'wait',--wait/pass/no
            const table_columns = [
                {
                    title: '申请编号',
...
export default class SubPagePTO extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
        ...
        serviceUser.getUserApplicationListByUsernameAndType(localStorage.getItem('user_name'), 'leave').then(data => {
            if (data != null) {
                this.setState({
                    dataList: data,
                    loading: false,
